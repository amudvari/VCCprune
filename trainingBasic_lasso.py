...
def pruneLoss(loss_fn, pred, y, prune_filter, budget, epsilon=1000):
    ...
    prune_filter_control = torch.exp( 1 * (sum(torch.square(torch.sigmoid(prune_filter_squeezed)))-budget)   )
...
for t in range(epochs):
    print(f"Epoch {t+1}\n-------------------------------")
    avg_error =  prune(train_dataloader, model1, model2, loss_fn, optimizer1, optimizer2, budget)
    avg_errors.append(avg_error)
    #test(test_dataloader, model, loss_fn)
    print("entire epoch's error: ", avg_error)
print("Done!")
